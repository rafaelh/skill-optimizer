...
class TestParseFrontmatterStrictFence:
    def test_closing_fence_with_trailing_text_not_matched(self):
    ...
    def test_opening_fence_with_trailing_text_rejected(self):
        text = "--- foo\nname: x\n---\nbody\n"
        fm, body = parse_frontmatter(text)
        assert fm is None
        assert body == text
    ...
    def test_closing_fence_at_eof(self):
        text = "---\nname: x\n---"
        fm, body = parse_frontmatter(text)
        assert fm == {"name": "x"}
        assert body == ""
...
class TestSanitizeForEcho:
    ...
    def test_strips_prompt_injection_marker_examples(self):
    ...
    def test_escapes_bidi_override_rlo(self):
    ...
    def test_escapes_all_bidi_overrides_and_isolates(self):
    ...
    def test_escapes_invisible_width_chars(self):
        for cp in (0x200B, 0x200C, 0x2060, 0xFEFF):
            ch = chr(cp)
            out = sanitize_for_echo(f"x{ch}y")
            assert ch not in out

    def test_preserves_zwj_for_emoji(self):
    ...
    def test_strips_single_byte_esc_sequence(self):
